...
from mitre_histology_toolkit.nuclei_density_window import nuclei_density_window as ndw
from mitre_histology_toolkit import nuclei_detection as nucdec
from mitre_histology_toolkit.image import inpainting
from mitre_multifractal_toolkit import mf_leaders as mf
...
class CD_single_tissue_piece:
    ...
    def run(self):
        ...
        if self.cd.use_nuc_features:    
            ...
            self.cd._print1(self.cd.nuclei_df)
            
            self.set_nuc_infos()
            self.cd._print2("%s: set_nuc_infos()" % self.monitorPrefix + 
                            ' ' + time.strftime("%H:%M:%S", time.gmtime()))
        if self.cd.use_nuclei_count * self.cd.use_mf:
            self.calc_nuc_density()
    ...
    def restrict_mask_to_single_tissue_piece(self):
        ...
        self.ymin, self.xmin = self.cd.ymin, self.cd.xmin
        self.ymax, self.xmax = self.cd.ymax, self.cd.xmax
        
        self.tissue_mask = self.tissue_mask[
            int(self.cd.ymin / F): int(self.cd.ymax / F),
            int(self.cd.xmin / F): int(self.cd.xmax / F)]
        print('xy_big', self.cd.xmin, self.cd.xmax, self.cd.ymin, self.cd.ymax)

        print('xy_small', self.xmin, self.xmax, self.ymin, self.ymax)
    ...
    def set_tissue_rgb(self):
        ...
        if self.cd.blend_nuc == False:
            self.cd._print1('not inpainting')
            self.tissue_rgb = self.cd.img.get_region(self.xmin, self.xmax, self.ymin, self.ymax, self.cd.MAG)
        else:
            self.cd._print1('inpainting')
            resize_factor = 20//self.cd.MAG
            st = ndw.tiles_to_img(self.cd.tile_dir, self.cd.mask_dir, resize_factor=resize_factor)
            _,_,_,filled = inpainting.grab_region_fill(st)
            
            if self.cd.anno_dir != 'tmp':
                pxt = round(abs(self.cd.xmin-self.xmin)/5)
                pxb = round(abs(self.cd.xmax-self.xmax)/5)
                pyl = round(abs(self.cd.ymin-self.ymin)/5)
                pyr = round(abs(self.cd.ymax-self.ymax)/5)

                channels = []
                for i in range(3):
                    channels.append(np.pad(filled[:,:,i], ((pyl,pyr),(pxt,pxb)), 'constant', constant_values = 255))
                self.filled = np.stack(channels,axis = 2)
                self.tissue_rgb = filled[self.xmin//resize_factor:self.xmax//resize_factor,self.ymin//resize_factor:self.ymax//resize_factor]
                
            else:    
                filled = inpainting.pad_nuc_fill(filled, self.cd.img.dimensions, resize_factor)
                self.filled = filled
                self.tissue_rgb = filled[self.ymin//resize_factor:self.ymax//resize_factor,self.xmin//resize_factor:self.xmax//resize_factor]
                print('filled', self.filled.shape)
    ...
    def set_superpixel_mask(self):
        ...
        if self.cd.use_grayscale:
            self.spixel_mask = slic(
                rgb2gray(self.tissue_rgb), n_segments=n_spixels, # mask = self.tissue_mask
                compactness=self.cd.compactness)
        else:
            self.spixel_mask = slic(
                self.tissue_rgb, n_segments=n_spixels, # mask = self.tissue_mask
                compactness=self.cd.compactness)
    ...
    def set_superpixel_features(self):
        ...
        assert (self.cd.use_intensity or self.cd.use_texture or self.cd.use_gradient or self.cd.use_mf)
        ...
        if self.cd.use_mf:
            ...
            for reg in rprops:
                bbox = reg.bbox
                im = tissue_htx[bbox[0]:bbox[2],bbox[1]:bbox[3]]

                try:
                    m=mf.get_mfSpectrum(im)
                    c1.append(m.C0p.c1[0])
                    c2.append(m.C0p.c2[0])
                except:
                    c1.append(np.nan)
                    c2.append(np.nan)
                    continue
            ...
            # c1 = np.array(c1)
            # c2 = np.array(c2)
            fdata_list.append(pd.DataFrame(list(zip(c1,c2)), columns=['c1','c2']))
            # self.cd._print1(fdata_list)
...
# =========================================================================
    def calc_nuc_density(self):
        assert (self.cd.use_nuclei_count* self.cd.use_mf)
        

        rgb_copy = self.tissue_rgb.copy()
        um_tiss = []
        px_height = self.cd.img.get_info()['mm_x']
        mag =  self.cd.MAG
        spx_lists = np.unique(self.spixel_mask)[1:]
        
        rprops = regionprops(self.spixel_mask)
        for i,r in enumerate(rprops):
            bbox = r.bbox
            tiss = rgb_copy[bbox[0]:bbox[2],bbox[1]:bbox[3]]
            spx = self.spixel_mask[bbox[0]:bbox[2],bbox[1]:bbox[3]]
            spx_label = spx_lists[i]

            tiss[np.where( spx != spx_label)] = 255
            perc_px_tiss = nucdec.tileseg.find_tissue(tiss)
            num_px_tiss = perc_px_tiss * tiss.shape[0] * tiss.shape[1]

            um_tiss.append(num_px_tiss[0] * px_height * mag+.00000001)
        self.fdata['mm_tiss'] = um_tiss
        self.fdata['nuc_dens'] = self.fdata.nuclei_count/self.fdata.mm_tiss
        self.fdata = self.fdata.drop(['mm_tiss','nuclei_count'], axis = 1)
        # self.cd._print1(self.fdata[self.fdata.isnull().any(axis=1)])

# =========================================================================

    def set_superpixel_assignment(self):
        self.fdata = self.fdata.apply(lambda x: x.fillna(x.mean()))
        self.cd._print1(self.fdata.isna().sum().sum())
        if self.cd.use_pca:
            scal = StandardScaler()
            ...
            self.UMAPComponents = pd.DataFrame(self.UMAPComponents,
                                               columns = [i + j for i, j in         zip(['umap']*self.cd.umap_comps,list(map(str,range(1,self.cd.umap_comps+1))))])
            if self.cd.use_db:
                self.cd._print1('hdbscan')

                clusterer = hdbscan.HDBSCAN(min_cluster_size=5,allow_single_cluster=True).fit(self.UMAPComponents)
                self.spixel_labels  = clusterer.labels_+1
...
class Cellularity_detector_superpixels (Base_HTK_Class):
    ...
    def __init__(self, img, **kwargs):
        ...
        use_gradient : bool
            ...
            intensity features.
        use_mf : bool
            Whether to extract c1 and c2 values using mitre_multifractal_toolkit
        keep_feats : list
        ...
        mask_dir : str
            path to masks
        blend_nuc : bool
            Whether to inpaint over nuclei 
        """
        default_attr = {
    ...
    def run(self):
        ...
        unique_tvals = list(set(np.unique(labeled)) - {0, })
        monitorPrefix = "%s: Number of segments detected %d" % (
            self.monitorPrefix, len(unique_tvals))
        self._print1(monitorPrefix)
        universal = CD_single_tissue_piece(self, tissue_mask=(labeled > 0), monitorPrefix=monitorPrefix)
        universal.run()
        del universal.tissue_rgb
        self.tissue_boundary = boundary_doc
        return universal
    ...
    def set_slide_info_and_get_tissue_mask(self):
        ...
        tloc = np.argwhere(labeled)
        F = self.slide_info['sizeX'] / labeled.shape[1]
        self.ymin, self.xmin = [int(j) for j in np.min(tloc, axis=0) * F]
        self.ymax, self.xmax = [int(j) for j in np.max(tloc, axis=0) * F]

        if self.anno_dir != 'tmp':
            print(self.anno_dir)
