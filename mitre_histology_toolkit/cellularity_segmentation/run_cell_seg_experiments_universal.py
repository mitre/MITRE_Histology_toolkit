...
from . import cell_seg_universal
from scipy.ndimage import label
import ast


def run_cell_seg_experiments_universal(path_to_slide_svs, path_to_nuclei_csv, path_to_output_csv , path_to_output_npz, 
                             path_to_tiss_seg_experiments,path_to_output_fdata, anno_dir, scene_name, path_to_output_mask):
    """      
    ...
    for index,row in experiments_df.iterrows():
        ...
        tissue_res.update({row['exper'] : cell_seg_universal.get_tissues(path_to_slide_svs, spixel_size_baseMag=64*64,MAG=4,
                                                                          ...
                                                                          anno_dir = anno_dir, image_name = scene_name)})
        ...
        tissue_piece = tiss['tissue_pieces']
        ...
        masks.append(label_mask.astype('uint8'))    
        # count the length of hetergenous borders 
        changes_sum.update( {row['exper']: np.sum(cell_seg_universal.count_heterogenous_border(label_mask))} )
    ...
    np.savez_compressed(f'{path_to_output_mask}', tissue_piece.spixel_mask)
    ...
    df_tmp['tot_border'] = np.sum(cell_seg_universal.count_heterogenous_border(re_spx))
    #add number of pixels in tissue
    df_tmp['num_px'] = (re_spx > 0).sum()
